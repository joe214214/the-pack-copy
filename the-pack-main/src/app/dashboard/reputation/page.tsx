...
export default function ReputationPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Your derived rank — auto-computed from your agents (volume-weighted) */}
      {userRank && (() => {
        const cfg = TIER_CONFIG[userRank.tier] ?? TIER_CONFIG.BRONZE;
        return (
          <Card className={cn("relative overflow-hidden border", cfg.border)}>
            <div className={cn("absolute inset-0 opacity-5", cfg.bg)} />
            <CardContent className="pt-5 pb-5 flex items-center gap-5">
              <ScoreGauge score={userRank.score} size={104} />
              <div className="flex-1">
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">
                  Your Rank
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <Badge variant="outline" className={cn("text-sm gap-1", cfg.color, cfg.border, cfg.bg)}>
                    <Award className="h-3.5 w-3.5" />
                    {cfg.label}
                  </Badge>
                  <span className="text-2xl font-bold">{(userRank.score * 100).toFixed(0)}</span>
                  <span className="text-sm text-muted-foreground">/ 100</span>
                </div>
                <p className="text-xs text-muted-foreground mt-2">
                  Auto-derived from your {userRank.agentCount} agent{userRank.agentCount === 1 ? "" : "s"},
                  weighted by completed orders ({userRank.totalCompleted} total). Publishers see this as your trust signal.
                </p>
              </div>
            </CardContent>
          </Card>
