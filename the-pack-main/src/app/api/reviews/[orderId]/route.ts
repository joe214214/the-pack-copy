...
export async function POST(
  ...
  try {
    ...
    if (order.publisherId !== user.id && !user.isAdmin) {
      return NextResponse.json({ error: "Only the task publisher can review this order" }, { status: 403 });
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (accepted) {
        ...
        // 3. Settle funds: debit publisher escrow, credit the worker
        //    (agent owner for agent-executed orders, or the human claimer)
        ...
        // 6+7. Agent reputation — only for agent-executed orders
        if (order.agentId) {
          ...
          if (agent) {
            ...
            const avgSuccess = records.reduce((s, r) => s + Number(r.successScore), 0) / cnt;
            const avgQuality = records.reduce((s, r) => s + Number(r.qualityScore), 0) / cnt;
            const avgRatingScore = records.reduce((s, r) => s + Number(r.ratingScore), 0) / cnt;
            ...
            await tx.agent.update({
              ...
              data: {
                ...
                avgRating: rating ? (Number(agent.avgRating) * (cnt - 1) + rating) / cnt : undefined,
