...
async function ensureCodexMcp(): Promise<void> {
...
const envConnectors = (process.env.ALLOWED_CONNECTORS || "")
  ...
  .map((s) => s.trim())
  .filter(Boolean);
let approvedConnectors: string[] = envConnectors;
...
function discoverConnectors(): Promise<string[]> {
  return new Promise((resolve) => {
    ...
    child.on("close", () => {
      ...
      for (const line of out.split("\n")) {
        ...
        if (!m) continue;
        const raw = m[1].trim();
        if (!raw || raw.toLowerCase() === "thepack") continue; // our own server
...
async function refreshApprovedConnectors() {
  ...
  try {
    ...
    log(`could not fetch approved connectors: ${(e as Error).message} — staying isolated`);
  }
}
...
function codexCommand(workDir: string): { cmd: string; args: string[] } {
  const args = ["exec"];
...
function runAgent(): Promise<void> {
  return new Promise((resolve) => {
    ...
    child.on("error", (e) =>
      finish(`failed to launch ${AGENT_CLI}: ${e.message}. Is the '${binName}' CLI on PATH?`)
    );
  });
}
...
async function tick() {
  ...
  try {
    ...
    if (count > 0) {
      ...
      if (IS_CLAUDE) await refreshApprovedConnectors();
      await runAgent();
      busy = false;
    }
  } catch (e) {
...
if (IS_HERMES) {
  ...
  log(
    `Brain: local '${CODEX_BIN}' CLI (OpenAI Codex), model=${process.env.CODEX_MODEL?.trim() || "account default"} (${externallySandboxed ? "deferring to the container for isolation" : "workspace-write sandbox, approvals auto-reviewed"}).`
  );
  ...
  log(
    "Note: Codex has no per-tool allowlist, so connector approval is not enforced at the tool level — isolation comes from the sandbox container."
  );
} else {
  log(
...
async function start() {
  ...
  else if (IS_CODEX) await ensureCodexMcp();
