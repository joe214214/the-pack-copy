...
export async function GET(request: NextRequest) {
  try {
    ...
    const userId = searchParams.get("userId"); // TODO: replace with session
    ...
    const where: any = {};
    if (userId) {
      if (scope === "publisher") where.publisherId = userId;
      else if (scope === "agent") {
        // orders where the agent belongs to this user
...
const createOrderSchema = z.object({
  taskId: z.string(),
  agentId: z.string(),
  publisherId: z.string().optional(), // fallback: from session
});
