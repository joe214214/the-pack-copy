...
## 18. The Apple web look — a concrete visual system for pages

The sections above are mostly about *motion*. This one is about *how an Apple page
actually looks*, so a static page (a landing/marketing/product page) reads as
unmistakably Apple and not as a generic "dark gradient = premium" guess. **The Apple
web aesthetic is bright, quiet, and enormous, not dark and busy.** Restraint is the
style: lots of white, one accent colour, huge type, huge space. When asked for an
"Apple-style" page, apply these tokens directly — don't approximate.

### Design tokens (use these values, not lookalikes)

```css
:root{
  /* Colour — light by default. Apple.com is white/near-white, NOT dark. */
  --ink:#1d1d1f;            /* near-black text, never pure #000 for body */
  --ink-2:#6e6e73;          /* secondary / subhead grey */
  --bg:#ffffff;             /* page base */
  --band:#f5f5f7;           /* the signature light-grey section band */
  --accent:#0066cc;         /* the single accent (links, primary button) */
  --hairline:rgba(0,0,0,.1);/* 1px separators, never heavy borders */
  /* Type — system font ships Apple's optical sizing already */
  --font:-apple-system,BlinkMacSystemFont,"SF Pro Display",system-ui,sans-serif;
  /* Radius — generous, soft */
  --r-card:20px; --r-panel:28px; --r-pill:980px;
  /* Shadow — low and soft, never hard */
  --shadow:0 4px 24px rgba(0,0,0,.08);
}
```

- **Type scale.** Hero headline `clamp(40px,6vw,80px)`, **weight 600** (semibold —
  Apple almost never uses 700+ black), `line-height:1.05`,
  `letter-spacing:-0.03em`. Section H2 `clamp(28px,4vw,48px)`, same treatment. Body
  `17–21px`, `line-height:1.47`, colour `--ink-2`. A small **eyebrow** label above a
  headline: `12–14px`, weight 600, sometimes in `--accent`.
- **Weight discipline.** Emphasis comes from weight 600 + size, not from black
  weights or ALL CAPS. Keep near-black `--ink` for text; never pure black.
- **Space is the design.** Section vertical padding `88–130px`. Centre content in a
  `max-width:980–1200px` column. When in doubt, add whitespace, not decoration.

### The six signature "tells" (these are what read on a projector)

1. **Frosted sticky nav** — ~48px tall, `backdrop-filter:blur(20px) saturate(180%)`,
   `background:rgba(255,255,255,.72)`, a `--hairline` bottom edge, small evenly-spaced
   links. Content scrolls *under* it.
2. **Oversized centred hero headline** on **two lines**, negative tracking, with a
   quiet grey subhead beneath — then the CTA row, then a large visual.
3. **The Apple CTA pair:** a **filled blue pill button** next to a **blue text link
   ending in a "›" chevron** ("Learn more ›"). Not two boxed buttons. The chevron
   link is the single most recognisable Apple tell.
4. **Alternating white / `--band` grey sections**, each with huge padding — the rhythm
   of white → grey → white is itself the Apple layout.
5. **One big rounded "product" panel** in the hero (a `--r-panel` gradient/image
   block with a soft shadow), treated as the star with room around it.
6. **Large rounded cards** (`--r-card`, `--shadow`, generous padding) with a **tinted
   rounded-square icon chip** at top-left of each.

### Components (drop-in specs)

```css
.btn{font:600 17px/1 var(--font);border-radius:var(--r-pill);padding:12px 22px;
     background:var(--accent);color:#fff;border:0;cursor:pointer;
     transition:transform .1s ease, background .2s ease}
.btn:active{transform:scale(.97)}                 /* §1 press feedback */
.link-arrow{color:var(--accent);font-size:17px;text-decoration:none}
.link-arrow::after{content:" ›";}                 /* the Apple chevron link */
.card{background:#fff;border-radius:var(--r-card);box-shadow:var(--shadow);
      padding:32px}
.nav{position:sticky;top:0;height:48px;background:rgba(255,255,255,.72);
     backdrop-filter:saturate(180%) blur(20px);border-bottom:1px solid var(--hairline)}
```

### Motion for a page (subtle, not springy)

A marketing page has no gestures, so §§1–11 barely apply. Use ONE restrained effect:
**scroll-reveal fade-up** — sections start `opacity:0; translateY(20px)` and settle
via `IntersectionObserver`, lightly staggered. Keep transitions `~500ms` ease-out on
`transform`/`opacity` only. Then honour §14: wrap it in
`@media (prefers-reduced-motion: reduce)` and drop the movement. Do **not** cover the
page in gradients or animations — the calm is the point.

### Anti-patterns that break the Apple look

- A **dark** background as the default (Apple marketing pages are light; dark is used
  only for a single dramatic "product" section, never the whole page).
- Two filled buttons instead of pill + chevron link.
- Heavy borders, hard shadows, black (700+) or ALL-CAPS headings, rainbow gradients.
- Cramped spacing. If it feels dense, it isn't Apple.
